...
char *Sys_BinaryPath(void)
{
 ...
 void logg(const char *errorStr, ...){
  ...
  __android_log_print(ANDROID_LOG_INFO, "quake", " %s ",error);
}
...
{
	logg("install path: %s", installPath);
	logg("Sys_Cwd: %s", Sys_Cwd());
	return "/mnt/sdcard/quake3";
	if (*installPath)
		return installPath;
	else
		return Sys_Cwd();
...
{
	__android_log_print(ANDROID_LOG_INFO, "quake", " %s ",msg);
...
static void* Sys_TryLibraryLoad(const char* base, const char* gamedir, const char* fname, char* fqpath )
{
	...
	*fqpath = 0;
	...
	fn = FS_BuildOSPath( "", gamedir, fname );
	...
	intptr_t (*systemcalls)(intptr_t, ...) )
{
	 ...
	 */
	 //niuzb
	libHandle = Sys_TryLibraryLoad(pwdpath, "/data/data/tk.niuzb.kkik3/lib", fname, fqpath);
...
{
	int   i;
	//char curdir[512]; not change the cur dir , or the dll load fail
	char  commandLine[ MAX_STRING_CHARS ] = { 0 };
    __android_log_print(ANDROID_LOG_DEBUG, "quake", "Inside Quake3 source!");
	//strcpy(curdir, "/sdcard/quake3/");
