...
int ifc_get_info(const char *name, struct sockaddr_in *addr, struct sockaddr_in *mask, unsigned *flags)
{
    struct ifreq ifr;
    ...
    if (mask != NULL) {
        if(ioctl(ifc_ctl_sock, SIOCGIFNETMASK, &ifr) < 0) {
            ...
            *mask = *((struct sockaddr_in*) (void*) &ifr.ifr_addr);
        }
    }
...
void NET_GetLocalAddress( void ) {
	...
	//todo: add NET_AddLocalAddress niuzb 2011.10.26
	
	struct sockaddr_in addr;
	if (ifc_init()) {
		return;
	}
	ifc_get_info("tiwlan0", &addr, &mask4, 0);
	//myAddr = addr;
	if(addr.sin_addr.s_addr != 0)	
		NET_AddLocalAddress("tiwlan0", (struct sockaddr *)&addr, (struct sockaddr *) &mask4);
	ifc_close();
