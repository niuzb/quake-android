...
extern 	void quake2_jni_key_event( int key, int down) ;
#define Key_Event quake2_jni_key_event
	...
	Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	*/
	...
	#if 0
	dest->x = dest->x*320/width;
	dest->y = dest->y*200/height;
	dest->w = dest->w*320/width;
	dest->h = dest->h*200/height;
	...
	glDrawTexiOES(dest->x, height - dest->y - dest->h, 0, dest->w, dest->h);
}
...
void init_screen_key_size()
{
...
#endif
	{
		...
		float iconscale = height/480.0f;
		int tempx, tempy;
		arrow.w = (int)(200.*iconscale);
		arrow.h = (int)(200.*iconscale);
		tempx = width/25+arrow.w/2;
		tempy = height*15/16 - arrow.h/2;
		...
		tempy = height/10+w/2;
        //consol
        setRect(&buttons[3], ((width-width/25)-w/2), tempy, w, w);
        //switch gun,just remove it
       // setRect(&buttons[1], width/30 + w/2, tempy, w, w);
        //jump
        setRect(&buttons[2], width-(int)(70.*iconscale)/2, height-(int)(80.*iconscale)/2, (int)(70.*iconscale), (int)(80.*iconscale));
        //fire
        setRect(&buttons[0], width-(int)(100.*iconscale), height*15/16 - arrow.h/2, (int)(120.*iconscale), (int)(120.*iconscale));
	}
}
...
void keybutton( JNIEnv*  env, jobject thiz, jbyteArray charBufJava )
{
	...
	unsigned char * charBuf = (unsigned char *) (*env)->GetByteArrayElements(env, charBufJava, &isCopy);
	int but, pos;
