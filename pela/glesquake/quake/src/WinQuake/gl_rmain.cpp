...
R_SetupFrame
===============
Retrieve the BSP leaf where the camera is, store it in variable "r_viewleaf"
...
********************************************/
	...
	R_RenderScene ();
	//(Render player model is we are in spectator mode
	R_DrawViewModel ();
	//(Switch to GL_BEND/GL_MODULATE mode to draw water. Warping is done 
	//via sin and cos lookup table from gl_warp.c
	R_DrawWaterSurfaces ();
...
//  End of all fog code...
	...
	// render mirror view
	R_Mirror ();
	//Blend the entire screen with value set in V_UpdatePalette via v_blend.
	//This is to show when we take damage (red), are under water or under
	//bonus boost effect
	R_PolyBlend ();
