...
static int setupScreenKeyboardButton( int buttonID, Uint8 * charBuf )
{
	...
	//__android_log_print(ANDROID_LOG_INFO, "quake", "On-screen keyboard generated OpenGL texture ID %x", data->id);
...
#if 0
	if(g_temp++ > 0xff) {
		LOGI("image:%d,%f,%f\n",arrowImages.id,arrowImages.w,arrowImages.h);
		LOGI("arrow:%d,%d,%d,%d\n", arrow.x, arrow.y, arrow.w,arrow.h);
		g_temp = 0;
	}
#endif
	beginDrawingTex();
...
void init_screen_key_size()
{
...
#endif
	{
		...
		tempx = scr_width/25+arrow.w/2;
		tempy = scr_height*15/16 - arrow.h/2;
